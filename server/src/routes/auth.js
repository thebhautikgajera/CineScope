...
const getUserWithCache = async (email) => {
    ...
    const user = await User.findOne({
        ...
        'password role lastLogin _id email phone tokenVersion failedLoginAttempts lockUntil isActive isEmailVerified'
    ).lean();
        ...
        await authLimiter(req, res, async () => {
            ...
            if (!email || !phone || !password) {
                return res.status(400).json({
                    ok: false,
                    error: 'Missing required fields: email, phone, password',
                });
            }
            ...
            // Format phone number: ensure it's 10 digits and add +91 prefix
            const phoneDigits = phone.trim().replace(/\D/g, ''); // Remove all non-digits
            ...
            // Check if user exists by email OR phone (permanent solution for duplicate key errors)
        ...
        await authLimiter(req, res, async () => {
            ...
            if (!userObj) {
                ...
                const doc = await User.findOne({
                    ...
                    .select('+password tokenVersion failedLoginAttempts lockUntil isActive role lastLogin email phone isEmailVerified')
                    .lean();
                userObj = doc;
